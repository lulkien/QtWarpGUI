...
Window {
  ...
  onVisibilityChanged: {
    ...
    case Window.Minimized:
      QML_Handler.qmlDebug(
            "[onVisibilityChanged] Visibility: Window.Minimized >>>>> root_window.hide()")
      root_window.hideWindow()
      break
    case Window.Hidden:
      QML_Handler.qmlDebug(
            "[onVisibilityChanged] Visibility: Window.Hidden >>>>> systrayVisibleChanged(true)")
      root_window.handleSystrayVisible(true)
      break
    default:
      QML_Handler.qmlDebug(
            "[onVisibilityChanged] Visibility: " + root_window.visibility
  ...
  function handleSystrayVisible(state) {
    QML_Handler.qmlSendEvent(
          state === true ? WarpEvents.EVT_SHOW_SYSTRAY : WarpEvents.EVT_HIDE_SYSTRAY)
